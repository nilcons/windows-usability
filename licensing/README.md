...
Windows 10 is free to download, but after 1-3 month of usage, you have
...
for and Microsoft created them for exactly these situations.
...
So MS basically has an hash function from the set of existing
...
following rules:
  ...
  - if `H(product_key)` is found, but `H(hardware)` is different, then
    ...
    couple of numbers of activations anyway (2-3).  If they accept it,
    they save the new `H(hardware)`,

  - if they have have rejected you in the previous point, you can just
    call in and they will allow it in exchange for wasting 10 mins of
    your time.  The phoneline is fully automated, no human interaction
...
Union decided at 2012-07-03 in the UsedSoft vs Oracle case that
...
If you want to obey to the OEM rules, you can buy from the US/UK
licenses which are really delivered to you (CD/DVD+sticker) and
according to the rules, you can pick-up a laptop in Los Angeles for 5
days.  These laptops are non-functional and they will be discarded if
you don't pick them up.  These licenses cost $20.
...
Once the upgrade is finished, with a 4 EUR code, you can use the
original Home license in e.g. a virtual machine, where disk encryption
is not needed.
...
Solution: eneric product keys" which you can Google
for and Microsoft created them for exactly these situations.
...
Basically you have to create a special installation USB media.
